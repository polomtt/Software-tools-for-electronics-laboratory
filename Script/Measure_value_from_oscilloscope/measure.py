...
instrument_resource_string = "TCPIP0::10.196.30.225::inst0::INSTR"
resource_manager = visa.ResourceManager()	# Opens the resource manager
my_instr = None 
my_instr = resource_manager.open_resource(instrument_resource_string)

instrument_write(my_instr,"MEASUrement:IMMed:SOUrce CH1")
instrument_write(my_instr,"MEASUREMENT:IMMED:TYPE PK2Pk")

try:
    while True:
        str_que = instrument_query(my_instr,"MEASUrement:IMMed:VALue?")
        print(str_que)
...
# while True:
#     instrument_query(my_instr,':MEAS:VOLT?')
